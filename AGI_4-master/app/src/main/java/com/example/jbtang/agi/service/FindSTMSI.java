...
import com.example.jbtang.agi.messages.ag2pc.L2P_MAC_DATA_STRU;
import com.example.jbtang.agi.messages.ag2pc.L2P_MAC_PDU_SUBHEADER_STRU;
import com.example.jbtang.agi.messages.ag2pc.L2P_PROTOCOL_DATA;
...
public class FindSTMSI {
    ...
    public Map<String,String> FnMap;
...
//            Toast.makeText(currentActivity, "SFN:--"+sfn+"--FN:--"+fn+"--", Toast.LENGTH_SHORT).show();
            Log.e(TAG,"SFN:--"+sfn+"--FN:--"+fn+"--STMSI:-"+ stmsi);
            if (fn!=-1){
                boolean flag=true;
                if (Global.Configuration.targetPhoneImsi.length()>0&&Global.Configuration.imsiflag){
                        ...
                        if(modfn>0){
                            if (4<=modfn&&modfn <16){
                                flag=false;
//                                Toast.makeText(currentActivity, "找到目标stmsi！", Toast.LENGTH_SHORT).show();
                            }
                        }else{
                            if(4<=modfn+128&&modfn+128<16){
                                flag=false;
//                                Toast.makeText(currentActivity, "找到目标stmsi！", Toast.LENGTH_SHORT).show();
                            }
                        }
    ...
    private  void resolveProtocolMsg(Global.GlobalMsg globalMsg){
        ...
        str.append("L2P_PROTOCOL_DATA:::::");
//        byte[] data=globalMsg.getBytes();
        int pos=0;
