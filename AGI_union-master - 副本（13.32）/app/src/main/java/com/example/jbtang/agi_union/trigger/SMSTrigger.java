...
public class SMSTrigger implements Trigger {
    ...
    @Override
    public void restar(Activity activity, Status.Service service) {
        Log.e("STMSITrigger","smscount=-1......");
        smsCount=-1;
        currentActivity = activity;
//        if (task != null && future != null) {
//            future.cancel(true);
...
//        future = null;
//        timer = null;
        for (final MonitorDevice device:DeviceManager.getInstance().getDevices()){
            if (device.getStatus()== Status.DeviceStatus.IDLE&&device.getCellInfo()!=null){
                Log.e("devicename",device.getName()+"11111111111");
    ...
    class OrientationFindingTask implements Runnable {
        ...
        public void run() {
            if (!startMonitor) {
                int i = 0;
                for (MonitorDevice device : DeviceManager.getInstance().getDevices()) {
                    if(Global.Configuration.model==Status.Model.CARRY || (Global.Configuration.model == Status.Model.VEHICLE && i==0)||Global.Configuration.OritationModle==1)//便携模式或 车载且M1
