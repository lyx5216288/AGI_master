...
public class OrientationFinding {
    ...
    private static final int COUNT_INTERVAL_CONTINUE = 4;//连续触发上报间隔 4改
    ...
    private Map<String,Timer> statusTimerMap;
...
//                    mOuter.get().resolveProlocolDataMsg(globalMsg);
//                    Log.i(TAG, "L2P_PROTOCOL_DATA_MSG_TYPE captured!!!!!!!!!!!!!!!!!!!!!!!");
//                    break;
                default:
                    break;
            }
    ...
    static class uiHander extends Handler{
//        private final WeakReference<OrientationFinding> mOuter1;
...
//            currentActivity.findViewById(R.id.orientation_find_start).setEnabled(true);
            currentActivity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(currentActivity, "目标脱标！", Toast.LENGTH_LONG).show();
                    OrientationFindingActivity.startButton.setEnabled(true);
                    stop();
...
//        }
    ...
    public void start(Activity activity) {
        ...
        } else if(orientationType == R.id.orientation_find_trigger_medium) {
            future = Global.ThreadPool.scheduledThreadPool.scheduleAtFixedRate(task, COUNT_INTERVAL_CONTINUE*2, COUNT_INTERVAL_CONTINUE*2, TimeUnit.SECONDS);
        ...
        for (MonitorDevice device : DeviceManager.getInstance().getDevices()) {
            if (device.getCellInfo() != null) {
                ...
                statusTimerMap.put(device.getName(),new Timer());
            }
        ...
        for (Map.Entry<String, Timer> entry : cellTimerMap.entrySet()) {
            entry.getValue().schedule(new CellTimerTask(entry.getKey()), 20000);
        }
...
//                }
        ...
        if(targetEarfcn!=null&&targetPci!=null){
            if (deviceflag&&cellInfo.earfcn==Integer.parseInt(targetEarfcn)&&cellInfo.pci==Integer.parseInt(targetPci)){
                MonitorDevice lastdevice;
                int xx=DeviceManager.getInstance().getDevices().size();
                if (xx>0){
                    lastdevice=DeviceManager.getInstance().getDevices().get(xx-1);
                    if (lastdevice.getName().equals(temDevice.getName())){
                        if (xx>1){
                            lastdevice=DeviceManager.getInstance().getDevices().get(xx-2);
                            lastdevice.setCellInfo(cellInfo);
                            cellTimerMap.put(lastdevice.getName(), new Timer());cellTimerMap.get(lastdevice.getName()).schedule(new CellTimerTask(lastdevice.getName()), 15000);
                            cellRSRPMap.put(lastdevice.getName(), new ArrayList<Float>());
                            lastdevice.startMonitor(Status.Service.ORIENTATION);

                            nextDeviceName = lastdevice.getName();
                        }else {
                            nextDeviceName="";
                        }
                    }else{
                        lastdevice.setCellInfo(cellInfo);
                        cellTimerMap.put(lastdevice.getName(), new Timer());
                        cellTimerMap.get(lastdevice.getName()).schedule(new CellTimerTask(lastdevice.getName()), 15000);
                        cellRSRPMap.put(lastdevice.getName(), new ArrayList<Float>());
                        lastdevice.startMonitor(Status.Service.ORIENTATION);
                        nextDeviceName = lastdevice.getName();
                    }
    ...
    public void stop() {
        ...
        for (Map.Entry<String, Timer> entry : statusTimerMap.entrySet()) {
            entry.getValue().cancel();
            entry.setValue(null);
        }
        cellTimerMap.clear();
        statusTimerMap.clear();
        task = null;
        future = null;
        stopCount();
    ...
    private void resolveUeCaptureMsg(Global.GlobalMsg globalMsg) {
        ...
        targetEarfcn=monitorDevice.getCellInfo().earfcn.toString();
        targetPci=monitorDevice.getCellInfo().pci.toString();
//        if (Global.Configuration.OritationModle==0&&false){
            //如果为便携模式或 车载模式且板卡为M1
//            if (Global.Configuration.model == Status.Model.CARRY || (Global.Configuration.model == Status.Model.VEHICLE && deviceName.equals(DeviceManager.getInstance().getDevices().get(0).getName()))) {
                if (orientationType == R.id.orientation_find_trigger_single) {
                    if (monitorDevice.getUeCaptureTimes() == 0) {
                        ...
                        monitorDevice.setUeCaptureTimes(2);
...
//                    int targetEarfcn=monitorDevice.getCellInfo().earfcn;
//                    short targetPci=monitorDevice.getCellInfo().pci;
//                    M1.getCellInfo().earfcn=targetEarfcn;
//                    M1.getCellInfo().pci=targetPci;
//                    monitorDevice.getCellInfo().earfcn=tempEarfcn;
//                    monitorDevice.getCellInfo().pci=tempPci;
//                }
...
//                Toast.makeText(currentActivity,  "缓存清空", Toast.LENGTH_SHORT).show();
    ...
    private void resolvePhyCommeasIndMsg(Global.GlobalMsg globalMsg) {
        ...
        if (statusTimerMap.get(temDevice.getName()) != null){
            statusTimerMap.get(temDevice.getName()).cancel();
            statusTimerMap.remove(temDevice.getName());
            statusTimerMap.put(temDevice.getName(),new Timer());
            statusTimerMap.get(temDevice.getName()).schedule(new CellTimerTask(temDevice.getName()),10000);
        }
    ...
    private void resolveProlocolDataMsg(Global.GlobalMsg globalMsg){
    ...
    private void resolveCellCaptureMsg(Global.GlobalMsg globalMsg) {
        ...
        if (statusTimerMap.get(monitorDevice.getName())!=null){
            Log.e(TAG,"statusTimerMap start!!!!");
            statusTimerMap.get(monitorDevice.getName()).schedule(new CellTimerTask(monitorDevice.getName()),10000);
        }
